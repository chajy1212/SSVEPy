...
def main(args):
    ...
    for subj in subjects:
        ...
        model_path = os.path.join(model_dir, f"Lee2019_Sub{subj}_EEGNet_{ch_tag}.pth")
        ...
        try:
            ...
            # Check dictionary structure (handle legacy vs new save formats)
            if "model_state" in checkpoint:
                eeg_branch.load_state_dict(checkpoint["model_state"]["eeg"])
                stim_branch.load_state_dict(checkpoint["model_state"]["stim"])
                temp_branch.load_state_dict(checkpoint["model_state"]["temp"])
                dual_attn.load_state_dict(checkpoint["model_state"]["attn"])
            else:
                # Direct keys in checkpoint
    ...
    # Print Final Results
    print(f"\n\n========== FINAL Time Window Analysis (Avg over {len(subjects)} subjects) ==========")
