...
# ===== Ablation Module: Simple Fusion Layer =====
class SimpleFusionLayer(nn.Module):
    def __init__(self, d_eeg, d_stim, d_temp, n_classes):
        super(SimpleFusionLayer, self).__init__()
        self.in_features = d_eeg + d_stim + d_temp

        print(
            f"[Model Init] Fusion Layer Input Dimension: {self.in_features} (EEG:{d_eeg} + Stim:{d_stim} + Temp:{d_temp})")
    ...
    def forward(self, eeg, stim, temp):
        # Global Average Pooling -> (Batch, Dim)
        if eeg.dim() > 2: eeg = self.pool(eeg).squeeze(-1)
        if stim.dim() > 2: stim = self.pool(stim).squeeze(-1)
        if temp.dim() > 2: temp = self.pool(temp).squeeze(-1)

        # Concatenation
        combined = torch.cat([eeg, stim, temp], dim=1)
...
# ===== Train / Eval =====
def train_one_epoch(eeg_branch, stim_branch, temp_branch, fusion_layer,
    ...
    for eeg, label in dataloader:
        ...
        # Simple Fusion
        logits, _, _, _ = fusion_layer(eeg_feat, stim_feat, temp_feat)
...
@torch.no_grad()
def evaluate(eeg_branch, stim_branch, temp_branch, fusion_layer,
...
def main(args):
    ...
    print(f"\n[Ablation Study] Running with Branches BUT Simple Fusion (No Attention)")
    ...
    for subj in subjects:
        ...
        print(f"[Check] EEGBranch Actual Output Dim: {actual_eeg_dim}")

        fusion_layer = SimpleFusionLayer(
        ...
        print_total_model_size(eeg_branch, stim_branch, temp_branch, fusion_layer)

        params = list(eeg_branch.parameters()) + list(stim_branch.parameters()) + \
                 list(temp_branch.parameters()) + list(fusion_layer.parameters())
        ...
        for epoch in range(1, args.epochs + 1):
            train_loss, train_acc = train_one_epoch(
                eeg_branch, stim_branch, temp_branch, fusion_layer,
            ...
            if test_acc > best_acc:
                ...
                save_path = os.path.join(save_dir, f"AblationDualLee2019_Sub{subj}_EEGNet_{ch_tag}.pth")

                torch.save({
                    "epoch": best_epoch,
                    "best_acc": best_acc,
                    "best_itr": best_itr,
                    "model_state": fusion_layer.state_dict(),
