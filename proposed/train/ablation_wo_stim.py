...
# ===== Subject parser =====
def parse_subjects(subjects_arg):
...
# ===== Ablation Module: Dual Attention (No Stimulus) =====
class DualAttention_NoStim(nn.Module):
    def __init__(self, d_eeg, d_temp, d_model, num_heads, proj_dim):
        super(DualAttention_NoStim, self).__init__()

        # Projections
        self.W_eeg = nn.Linear(d_eeg, d_model)
        self.W_temp = nn.Linear(d_temp, d_model)

        # Multi-Head Attention (EEG <-> Template)
    ...
    def forward(self, eeg, temp):
        ...
        # Permute EEG dimensions to (B, T, De) for MultiheadAttention
        if eeg.dim() == 3:
            eeg = eeg.permute(0, 2, 1)

        # Permute Template dimensions to (B, T, Dt)
        # If input is 2D (Batch, Dim), expand it to 3D (Batch, Time, Dim)
        if temp.dim() == 2:
            # Expand (B, Dt) -> (B, 1, Dt) -> (B, T, Dt) to match time axis
            temp = temp.unsqueeze(1).expand(-1, eeg.size(1), -1)
        elif temp.dim() == 3:
            temp = temp.permute(0, 2, 1)

        # Linear Projection
        Q = self.W_eeg(eeg)    # (B, T, D_model)
        K = self.W_temp(temp)  # (B, T, D_model)
        V = K

        # Attention (Query: EEG, Key/Value: Template)
...
# ===== Train / Eval =====
def train_one_epoch(eeg_branch, temp_branch, attention_module,
    ...
    for eeg, label in dataloader:
        ...
        if eeg.dim() == 3: eeg = eeg.unsqueeze(1)
        ...
        logits, _, _, _ = attention_module(eeg_feat, temp_feat)
...
@torch.no_grad()
def evaluate(eeg_branch, temp_branch, attention_module,
...
def main(args):
    ...
    print(f"\n[Ablation Study] Removing Stimulus Branch (Template Only Mode)")
    ...
    for subj in subjects:
        ...
        with torch.no_grad():
            dummy_out = eeg_branch(dummy_input, return_sequence=True)
            actual_eeg_dim = dummy_out.shape[1]

        attention_module = DualAttention_NoStim(
            d_eeg=actual_eeg_dim,
            d_temp=args.d_query,
        ...
        print_total_model_size(eeg_branch, temp_branch, attention_module)

        params = list(eeg_branch.parameters()) + list(temp_branch.parameters()) + \
        ...
        for epoch in range(1, args.epochs + 1):
            train_loss, train_acc = train_one_epoch(
                eeg_branch, temp_branch, attention_module,
                train_loader, optimizer, ce_criterion, device
            )
            test_loss, test_acc, itr = evaluate(
                eeg_branch, temp_branch, attention_module,
                test_loader, ce_criterion, device, n_classes, trial_time
            )
            scheduler.step()

            print(
                f"\n[Epoch {epoch:03d}] Train Loss: {train_loss:.5f} | Train Acc: {train_acc:.5f} || Test Loss: {test_loss:.5f} | Test Acc: {test_acc:.5f} | ITR: {itr:.4f}")
            ...
            if test_acc > best_acc:
                ...
                save_path = os.path.join(save_dir, f"AblationStimLee2019_Sub{subj}_EEGNet_{ch_tag}.pth")

                torch.save({
                    "epoch": best_epoch,
                    "best_acc": best_acc,
                    "best_itr": best_itr,
                    "model_state": attention_module.state_dict(),
                    "optimizer": optimizer.state_dict(),
                }, save_path)

                print(f"\n[Save] Epoch {best_epoch} → Best model (Acc={best_acc:.5f}) saved")

        all_accs.append(best_acc)
        all_itrs.append(best_itr)

    print("\n[Final Ablation Result (No Stimulus)]")
